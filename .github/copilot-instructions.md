...
- `evaluation/` – Orchestrated by `evaluation.evaluator.evaluate_segmentation`. Produces a dict keyed by methods enumerated in `config/constants.py::EVAL_METHODS` (`nuclei`, `syll_boundaries`, `syll_spans`, `word_boundaries`, `word_spans`). Boundary & span evaluators compare predicted spans against reference intervals extracted from TextGrid tiers (or generated placeholder). Each metric dict contains counts like `TP`, `Ins`, `Del`, `Sub`.
...
- `config/` – Static constants: evaluation method names (`EVAL_METHODS`), vowel sets (`SYLLABIC`), default tolerance.
...
- `EVAL_METHODS` order matters for flatten/aggregation loops; if you add a new evaluation dimension, append its key there and ensure `evaluate_segmentation` populates it (or `flatten_results` will skip missing entries silently).
...
## 8. Common Pitfalls
- Passing `syllable_tier` expecting list: `evaluate_segmentation` handles `-1` (generate placeholder) or None (skip). Ensure callers supply consistent indices (0-based tier ordering from `textgrid` lib).
- `extract_syllable_intervals` returns dict; evaluation code currently calls it for words too. If refactoring, centralize normalization (either always return list or adapt evaluation accordingly).
- Ensure new envelope methods produce time arrays identical length to envelope to keep peak picking accurate.
- Avoid modifying global constants names; `flatten_results` iterates `EVAL_METHODS`.
...
results_df = run_evaluation('data/**/*.TextGrid', 'data/**/*.wav', phone_tier=1, syllable_tier=2, word_tier=3, envelope_fn='sbs')
```
Aggregate:
```python
...
- Update `EVAL_METHODS` if adding evaluator key; ensure `evaluate_segmentation` populates it.
